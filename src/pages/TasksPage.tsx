...
export function TasksPage() {
  ...
  const { tasks, loading, error, loadTasks, addTask, editTask, removeTask, setTaskStatus } =
    useTasks();

  const { filters, setFilter, clearFilters } = useUrlFilters();
  const [sort, setSort] = useState<TaskSort>(DEFAULT_SORT);

  const { pageTasks, totalFiltered, totalPages } = useFilteredTasks(tasks, filters, sort);
  ...
  const openCreateForm = (): void => setFormState({ open: true, mode: 'create' });
  const openEditForm = (task: Task): void => setFormState({ open: true, mode: 'edit', task });
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
      ...
      <ConfirmDialog
        ...
        message={taskToDelete ? `"${taskToDelete.title}" will be permanently deleted.` : ''}
        confirmLabel="Delete"
