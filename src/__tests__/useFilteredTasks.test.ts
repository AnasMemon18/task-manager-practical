...
describe("useFilteredTasks", () => {
  it("returns all tasks when no filters are active", () => {
    ...
    const { result } = renderHook(() =>
      useFilteredTasks(tasks, defaultFilters, defaultSort),
    );
  ...
  it("filters by search across title and description", () => {
    ...
    const { result } = renderHook(() =>
      useFilteredTasks(tasks, filters, defaultSort),
    );
  ...
  it("sorts by priority descending (high first)", () => {
    ...
    const { result } = renderHook(() =>
      useFilteredTasks(tasks, defaultFilters, sort),
    );

    expect(result.current.pageTasks.map((t) => t.priority)).toEqual([
      "High",
      "Medium",
      "Low",
    ]);
